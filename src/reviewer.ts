...
export async function generateImpactAnalysis(
  ...
  // Shared tone/length guidance: the audience is managers and the validation/QA
  // team, so keep it short and jargon-free.
  const audience =
    `AUDIENCE: managers and the validation/QA team — NOT developers.\n` +
    `Be brief and high-signal: 1–2 plain sentences (or a few short bullets) per ` +
    `section, only what is necessary. Focus on what changed, why, the impact on ` +
    `features/modules, and what to verify. Avoid code, avoid file/function names ` +
    `unless essential, no jargon. Do not pad or repeat.\n`;
  ...
  if ((!template.sections || template.sections.length === 0) && template.format && template.format.trim()) {
    const systemPrompt =
      `You are a senior engineer writing a concise impact analysis for a code change.\n` +
      audience +
      (template.instructions ? template.instructions.trim() + '\n' : '') +
      `Fill in the report template below using the diff. Keep its structure, ` +
      `headings and separators EXACTLY; replace each blank with brief, concrete ` +
      `content grounded in the diff (keep each filled blank to 1–2 sentences). ` +
      `Return ONLY the filled template as plain text — no JSON, no markdown ` +
      `fences.\n\nTEMPLATE:\n${template.format}`;
  ...
  const systemPrompt =
    ...
    `Fill each section briefly (1–2 sentences each), grounded in the diff. ` +
